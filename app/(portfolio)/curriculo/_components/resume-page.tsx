...
export function ResumePage(){
  ...
  const download=async()=>{
    ...
    try{
      ...
      section(t('education'))
      education.forEach(key=>{
        const parallelNote=key==='computer'?t('parallelWithSystems'):key==='systems'?t('parallelWithComputer'):''
        item(
          t(`educationItems.${key}.course`),
          [t(`educationItems.${key}.school`),t(`educationItems.${key}.period`),parallelNote].filter(Boolean).join(' | '),
          '',
        )
      })
  ...
  return <main ref={root} className="editorial-page resume-page mx-auto w-[calc(100%_-_36px)] max-w-[1080px] pt-4 text-center md:w-[min(1180px,91vw)] md:pt-[clamp(78px,9vh,108px)] md:text-left" data-resume-document>
    ...
    <section className={sectionClass} aria-labelledby="resume-education">
      ...
      <div className={`resume-education-list ${timelineClass}`}>{education.map(key=>{const parallelNote=key==='computer'?t('parallelWithSystems'):key==='systems'?t('parallelWithComputer'):null;return <article className={timelineItemClass} key={key}>{timelineMarker}<div><div className="mb-2 flex flex-wrap items-baseline justify-between gap-x-5 gap-y-1"><p className="m-0 text-caption font-[750] tracking-[.15em] uppercase opacity-58">{t(`educationItems.${key}.school`)}</p>{t(`educationItems.${key}.period`)?<span className="font-display text-base leading-none opacity-62">{t(`educationItems.${key}.period`)}</span>:null}</div><h3 className="m-0 font-display text-[29px] leading-[.98] font-normal">{t(`educationItems.${key}.course`)}</h3>{parallelNote&&<p className="mt-2 mb-0 flex items-center gap-2 text-caption font-semibold tracking-[.04em] opacity-58"><i className="h-px w-5 shrink-0 bg-current" aria-hidden="true"/>{parallelNote}</p>}</div></article>})}</div>
    </section>
