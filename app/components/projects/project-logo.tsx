...
export function ProjectLogo({id,title,className='',framed=true,compact=false}:{id:string;title:string;className?:string;framed?:boolean;compact?:boolean}){
  ...
  const compactStyle=compact?{width:'48px',minWidth:'48px',maxWidth:'48px',height:'36px',minHeight:'36px',maxHeight:'36px',alignSelf:'center'}:undefined
  const imageStyle=compact?{display:'block',width:'48px',height:'30px',maxWidth:'48px',maxHeight:'30px',objectFit:'contain' as const,objectPosition:'center'}:undefined
  return <div className={`project-logo grid place-items-center ${frame} ${className}`} style={compactStyle}>
