...
export function GlobalProfile(){
  ...
  return <>
    <aside className="global-profile global-profile--corner scene-item relative z-30 mx-auto mt-6 mb-[calc(32px+env(safe-area-inset-bottom))] hidden w-[min(calc(100%_-_48px),300px)] border-t border-[color-mix(in_srgb,var(--paper)_32%,transparent)] pt-6 text-center text-[color-mix(in_srgb,var(--paper)_92%,transparent)] md:fixed md:top-[26px] md:right-[4.5vw] md:m-0 md:block md:w-[min(21vw,250px)] md:border-0 md:pt-0 md:text-right md:will-change-[transform,opacity] md:max-[1100px]:w-[210px]">
      ...
      <p className="global-profile__introduction mx-auto my-2 max-w-[34ch] text-small leading-[1.45] font-[540] opacity-78 md:mt-[15px] md:mr-0 md:mb-3.5 md:ml-auto md:w-full md:max-w-none md:whitespace-pre-line md:text-[11px] md:leading-[1.55] md:max-[1100px]:text-[10px]"><TypedText text={profile('introduction')}/></p>
    ...
    <footer id="contato" className={`global-profile relative z-10 mx-auto mt-0 w-[calc(100%_-_36px)] px-1 pt-0 text-paper transition-[padding-bottom] duration-340 ease-[cubic-bezier(.22,1,.36,1)] md:mt-12 md:w-[calc(100%_-_9vw)] md:border-t md:border-[color-mix(in_srgb,var(--paper)_22%,transparent)] md:px-0 md:pt-5 ${navPosition==='bottom'?'pb-[calc(88px+env(safe-area-inset-bottom))] md:pb-[88px]':'pb-[calc(24px+env(safe-area-inset-bottom))] md:pb-5'}`}>
      <div className="mx-auto flex w-full max-w-[420px] flex-col items-center border-b border-[color-mix(in_srgb,var(--paper)_28%,transparent)] px-1 py-6 text-center md:hidden">
