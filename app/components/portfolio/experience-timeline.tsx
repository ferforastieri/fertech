...
export function ExperienceTimeline({navPosition}:{navPosition:NavPosition}){
  ...
  return <section ref={root} id="experiencia" className={`experience-section relative min-h-0 px-[18px] pt-4 pb-6 md:min-h-svh md:px-[4.5vw] md:pt-[clamp(110px,14vh,170px)] md:pb-[120px] ${navPosition==='top'?'scroll-mt-[52px] md:scroll-mt-0':''}`} aria-labelledby="experience-title">
    ...
    <Link className="experience-resume group mx-auto mt-9 grid max-w-[720px] grid-cols-[1fr_auto] items-center gap-x-[18px] gap-y-1.5 border-t border-[color-mix(in_srgb,var(--paper)_34%,transparent)] px-4 py-5 text-left text-inherit no-underline opacity-0 focus-visible:outline focus-visible:outline-offset-5 focus-visible:outline-current md:mt-[90px] md:gap-x-[30px] md:gap-y-[7px] md:border-b md:px-7 md:py-6" href="/curriculo"><span className="col-start-1 m-0 text-caption font-[720] tracking-[.16em] uppercase opacity-52">{t('resumeHint')}</span><strong className="col-start-1 font-display text-[28px] leading-[.95] font-normal md:text-[clamp(27px,3vw,40px)]">{t('resumeCta')}</strong><i className="col-start-2 row-start-1 row-end-3 font-display text-[35px] not-italic transition-transform duration-250 group-hover:translate-x-1 group-hover:-translate-y-1" aria-hidden="true">↗</i></Link>
  </section>
}
