...
export function ExperienceTimeline({navPosition}:{navPosition:NavPosition}){
  ...
  return <section ref={root} id="experiencia" className={`experience-section relative min-h-0 px-[18px] pt-4 md:min-h-svh md:px-[4.5vw] md:pt-[clamp(110px,14vh,170px)] ${navPosition==='top'?'scroll-mt-[52px] md:scroll-mt-0':''}`} aria-labelledby="experience-title">
    ...
    <div className="timeline-body relative mx-auto ml-0 max-w-[1060px]"><div className="timeline-spine absolute top-1.5 bottom-1.5 left-1 w-px bg-[color-mix(in_srgb,var(--paper)_15%,transparent)] md:left-1/2" aria-hidden="true"><span className="timeline-spine-progress block h-full w-full origin-top bg-paper"/></div>
    ...
    <Link className="experience-resume group relative mx-auto mt-9 flex max-w-[720px] flex-col items-center gap-y-1.5 border-y border-[color-mix(in_srgb,var(--paper)_34%,transparent)] px-14 py-5 text-center text-inherit no-underline opacity-0 focus-visible:outline focus-visible:outline-offset-5 focus-visible:outline-current md:mt-[90px] md:gap-y-[7px] md:px-16 md:py-6" href="/curriculo"><span className="m-0 text-caption font-[720] tracking-[.16em] uppercase opacity-52">{t('resumeHint')}</span><strong className="font-display text-[28px] leading-[.95] font-normal md:text-[clamp(27px,3vw,40px)]">{t('resumeCta')}</strong><i className="absolute top-1/2 right-4 -translate-y-1/2 font-display text-[35px] not-italic transition-transform duration-250 group-hover:translate-x-1 group-hover:-translate-y-[calc(50%+4px)] md:right-7" aria-hidden="true">↗</i></Link>
  </section>
}
