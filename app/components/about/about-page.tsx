...
const stack=['React','Next.js','TypeScript','Node.js','NestJS','React Native','PostgreSQL','Docker','IA aplicada','Design Systems']
...
export function AboutPage(){
  ...
  return <SiteFrame><div ref={root} className="editorial-page about-page">
    ...
    <section className="about-stack about-reveal" aria-labelledby="about-stack-title"><div><p>{t('stackEyebrow')}</p><h2 id="about-stack-title">{t('stackTitle')}</h2><span>{t('stackDescription')}</span></div><ul>{stack.map(item=><li key={item}>{item}</li>)}</ul></section>
